 ...
 * P_481
 * !!!
 * some class have no move-constructor and move-assignment: !!!
 *	compiler won't generate synthesized move-constructor and synthesized
 *	move-assignment for some class, especialy for class which have
 *	custom copy-constructor, copy-assignment or destructor;
 *	only class have no copy control member function and all non-static
 *	member can be move which can be generated synthesize move operation;
 * when copy-assignment and move-assignment both exist: !!!
 *	lvalue using copy-assignment
 *		v1 = v2;	// variables are lvalue
 *	rvalue using move-assignment
 *		int foo();	// return rvalue
 *		v1 = foo();
 * copy-constructor, copy-assignment, move-constructor, move-assignment, destructor
 * should be considered as a whole, one customize, all customize
 */
#include <iostream>
#include <fstream>
#include <sstream>
